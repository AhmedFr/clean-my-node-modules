/* landing.js — pixel meters, framework row builder, scroll reveals, nav state. */
(function () {
  "use strict";
  ...
  // ---------- framework rows ----------
  var FW = {
    react: "legacy-dashboard", next: "checkout-flow", vue: "portfolio-2021",
    svelte: "svelte-playground", node: "hackathon-bot", expo: "recipe-app-old"
  };
  function row(kind, name, path, age, size, unit, hl) {
    return '<div class="ui-row' + (hl ? " hl" : "") + '">' +
      '<svg class="fw" viewBox="0 0 24 24"><use href="#fw-' + kind + '"></use></svg>' +
      '<div class="meta"><div class="nm">' + name + '</div>' +
      '<div class="sub pathmono">' + path + '</div></div>' +
      '<div class="age">' + age + '</div>' +
      '<div class="sz">' + size + '<span class="u">' + unit + '</span></div></div>';
  }
